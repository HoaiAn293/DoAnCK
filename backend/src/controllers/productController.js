...
exports.createProduct = async (req, res) => {
  try {
    const { name, description, price, duration, categoryId, thumbnail, images, ingredients } = req.body;
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    // ===== VALIDATION =====
    if (!name && !price && !duration && !categoryId && !description && (thumbnail === undefined || thumbnail === null || thumbnail === '') && !images && !ingredients) {
      return res.status(400).json(
    ...
    } else if (thumbnail !== undefined && thumbnail !== null && thumbnail !== '') {
      // URL string from frontend
